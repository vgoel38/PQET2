...
def create_table(cur, table, varchar_length):
    sql = "CREATE TABLE " + table + "(num    INT     NOT NULL, pad VARCHAR(" + str(varchar_length) + "));"
    execute_sql(cur, sql)
...
def insert_data(cur, table):
    sql = "COPY " + table + " FROM '/home/sahana/Documents/PQET/Prediction-of-Query-Execution-Time/sort/nums.csv' DELIMITER ' ';"
    execute_sql(cur, sql)
...
def run_query(cur, table, attr):
    sql = "EXPLAIN (ANALYSE, BUFFERS) SELECT * FROM " + table + " ORDER BY "+ attr + ";"
    execute_sql(cur, sql)
