...
def find_range(cur, table, x):
    sql = "select max(movie_id) from (select movie_id from cast_info where movie_id>0 limit "+str(i)+") as temp;"
    execute_sql(cur, sql)
    result = cur.fetchall()[0][0]
    return result

def find_card(cur, table, x):
    sql = "select count(*) from cast_info where movie_id>0 and movie_id<=" + str(x);
    execute_sql(cur, sql)
    result = cur.fetchall()[0][0]
    return result
...
if __name__ == "__main__":
	...
	for i in range(5400023,36244345,1000000):
		
		rang = find_range(cur,'cast_info',i)
		card = find_card(cur,'cast_info',rang)
		ranges.append(rang)
		cards.append(card)
		print(rang, card)
	...
	np.savetxt('/home/dsladmin/Documents/vishal/index_scan/vuc_model_0/ranges.txt',ranges,delimiter=',');
	np.savetxt('/home/dsladmin/Documents/vishal/index_scan/vuc_model_0/cards.txt',cards,delimiter=',');
